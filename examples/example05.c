...
static const char TEST_STRING[] = "Genes exist to pass down our hopes and dreams for the future through our children. "
                                  "Living is a link to the future. That's how all life works. Loving each other, teaching each other... "
                                  "that's how we can change the world. I finally realized it. The true meaning of life...";
...
int main(void)
{
    ...
    r = qryptext_encrypt((uint8_t*)TEST_STRING, TEST_STRING_LENGTH, encrypted_string, sizeof(encrypted_string), &encrypted_string_length, false, TEST_PUBLIC_KEY);

    printf("Status code: %d\n\n", r);

    if (r != 0)
    {
        printf("ERROR: Encryption failed! \"qryptext_encrypt\" returned: %d", r);
        goto exit;
    }

    printf("Encrypted string >>> raw binary:\n\n%s\n\n", encrypted_string);

    r = qryptext_decrypt(encrypted_string, encrypted_string_length, false, decrypted_string, sizeof(decrypted_string), &decrypted_string_length, TEST_SECRET_KEY);

    printf("Status code: %d\n\n", r);

    if (r != 0)
    {
        printf("ERROR: Decryption failed! \"qryptext_decrypt\" returned: %d", r);
        goto exit;
    }
